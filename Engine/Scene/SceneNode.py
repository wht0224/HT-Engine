...
class SceneNode:
    ...
    def get_position(self):
        return self.position.copy() if self.position else Vector3(0, 0, 0)

    def get_rotation(self):
        return self.rotation.copy() if self.rotation else Quaternion.identity()

    def get_scale(self):
        return self.scale.copy() if self.scale else Vector3(1, 1, 1)
    ...
    def update_transform(self):
        ...
        if self.parent:
            ...
            # 修复：Vector3 * Vector3 在 Math 库中被定义为点积（返回 float），导致 world_scale 变成 float
            # 这里我们需要逐元素相乘（Hadamard product）
            ps = self.parent.world_scale
            ls = self.scale
            # 检查父节点缩放是否意外变成了 float (防御性编程)
            if isinstance(ps, (int, float)):
                self.world_scale = Vector3(ps * ls.x, ps * ls.y, ps * ls.z)
            else:
                self.world_scale = Vector3(ps.x * ls.x, ps.y * ls.y, ps.z * ls.z)
        else:
            # 如果没有父节点，局部变换就是世界变换
