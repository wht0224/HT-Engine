...
# 导入优化的视锥体类
try:
    from Engine.Optimizations.FastFrustum import OptimizedFrustum
    USE_OPTIMIZED_FRUSTUM = True
except ImportError:
    USE_OPTIMIZED_FRUSTUM = False
...
class Camera:
    ...
    def __init__(self, name="MainCamera"):
        ...
        # 视锥体
        if USE_OPTIMIZED_FRUSTUM:
            self.frustum = OptimizedFrustum()
            print(f"Camera '{self.name}': Using OptimizedFrustum for better performance")
        else:
    ...
    def _update_rotation_from_look_at(self):
        """根据look_at目标更新旋转 - 使用标准gluLookAt算法"""
        ...
        # 构建相机到世界的旋转矩阵 (列向量为Right, Up, Back)
        # Col 0 = Right
        rot_matrix[0, 0] = right.x; rot_matrix[1, 0] = right.y; rot_matrix[2, 0] = right.z
        # Col 1 = Up
        rot_matrix[0, 1] = up.x;    rot_matrix[1, 1] = up.y;    rot_matrix[2, 1] = up.z
        # Col 2 = Back (-Direction)
        rot_matrix[0, 2] = -direction.x; rot_matrix[1, 2] = -direction.y; rot_matrix[2, 2] = -direction.z
