...
class OpenGLViewport(OpenGLFrame if HAS_PYOPENGLTK else tk.Frame):
    ...
    def __init__(self, master, engine, **kwargs):
        ...
        # 检查是否为游戏或飞行模拟器模式
        use_engine_renderer = False
        if engine and hasattr(engine, 'config'):
            ui_type = engine.config.get("frontend", {}).get("ui_type", "default")
            use_engine_renderer = (ui_type == "flight_simulator" or ui_type == "game")

        # 渲染状态
        self.is_initialized = False
        self.render_grid = not use_engine_renderer  # 游戏模式下不渲染网格
        self.render_axes = not use_engine_renderer  # 游戏模式下不渲染坐标轴
        self.render_example_cube = not use_engine_renderer  # 游戏模式下不渲染示例立方体
    ...
    def initgl(self):
        ...
        try:
            import moderngl
            # moderngl.create_context() attaches to the currently active context
            self.ctx = moderngl.create_context()
            self.logger.info("ModernGL上下文已创建 (Attached to Tkinter)")
        except Exception as e:
            self.logger.warning(f"无法创建ModernGL上下文: {e}")
            self.ctx = None
    ...
    def redraw(self):
        ...
        if self.engine and hasattr(self.engine, 'config'):
            ui_type = self.engine.config.get("frontend", {}).get("ui_type", "default")
            use_engine_renderer = (ui_type == "flight_simulator" or ui_type == "game")

        if use_engine_renderer:
            # 硬编码蓝色天空
            clear = (0.3, 0.5, 0.8, 1.0)
            glClearColor(*clear)
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
            if not hasattr(self, "_bg_probe_done"):
                try:
                    px = glReadPixels(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE)
                    print(f"[ViewportProbe] clear={clear} RGBA={list(px)}")
                except Exception:
                    pass
                self._bg_probe_done = True
            # 游戏或飞行模拟器模式：让引擎完全控制渲染
            # 引擎会负责设置投影、视图矩阵以及渲染所有内容
            try:
                # 同步视口大小到引擎平台，确保Engine.render使用正确的视口大小
                if hasattr(self.engine, 'platform') and self.engine.platform:
                    self.engine.platform.width = self.width
                    self.engine.platform.height = self.height
                # 兼容旧代码，有些地方可能使用plt别名
                if hasattr(self.engine, 'plt') and self.engine.plt:
                    self.engine.plt.width = self.width
                    self.engine.plt.height = self.height
    ...
    def _setup_modelview(self):
        ...
        # 检查是否为飞行模拟器模式，使用引擎相机
        use_engine_camera = False
        if self.engine and hasattr(self.engine, 'config'):
            ui_type = self.engine.config.get("frontend", {}).get("ui_type", "default")
            use_engine_camera = (ui_type == "flight_simulator")

        if use_engine_camera and hasattr(self.engine, 'scene_mgr') and self.engine.scene_mgr.active_camera:
            # 飞行模拟器模式：使用引擎的相机系统
            camera = self.engine.scene_mgr.active_camera
    ...
    def _render_scene_objects(self):
        ...
        elif hasattr(self, 'render_example_cube') and self.render_example_cube:
            # 仅在编辑器模式下渲染默认的示例立方体
            glPushMatrix()
    ...
    def _on_mouse_motion(self, event):
        """鼠标移动事件（游戏模式 - 无按键拖拽）"""
        # 检查是否为游戏模式
        if self.engine and hasattr(self.engine, 'tk_ui') and self.engine.tk_ui:
            ui = self.engine.tk_ui
            if hasattr(ui, 'input_state') and getattr(ui, 'mouse_locked', False):
                # 计算相对于屏幕中心的偏移
                center_x = ui.width // 2
                ...
                if dx != 0 or dy != 0:
                    ui.input_state['mouse_dx'] += dx
                    ui.input_state['mouse_dy'] += dy
                    
                    # 将鼠标移回中心
                    try:
                        self.event_generate('<Motion>', warp=True, x=center_x, y=center_y)
                    except Exception:
                        pass
            ...
            if self.is_initialized:
                ...
                try:
                    if self.engine and hasattr(self.engine, "platform") and self.engine.platform:
                        self.engine.platform.width = self.width
                        self.engine.platform.height = self.height
                    if self.engine and hasattr(self.engine, "plt") and self.engine.plt:
                        self.engine.plt.width = self.width
                        self.engine.plt.height = self.height
                    if self.engine and hasattr(self.engine, "renderer") and self.engine.renderer and hasattr(self.engine.renderer, "resize"):
                        self.engine.renderer.resize(self.width, self.height)
                    if self.engine and hasattr(self.engine, "scene_mgr") and self.engine.scene_mgr and getattr(self.engine.scene_mgr, "active_camera", None):
                        cam = self.engine.scene_mgr.active_camera
                        if hasattr(cam, "set_aspect_ratio"):
                            aspect = (self.width / self.height) if self.height > 0 else 1.0
