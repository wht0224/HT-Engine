...
class RenderPipelineOptimizer:
    ...
    def _adjust_render_quality(self):
        ...
        if self.renderer.quality != quality:
            ...
            if hasattr(self.renderer, "set_render_quality"):
                self.renderer.set_render_quality(quality)
            else:
    ...
    def _adjust_pipeline_parameters(self):
        ...
        # 调整阴影贴图分辨率
        self.renderer.shadow_map_res = self.recommended_settings["shadow_map_resolution"]
        ...
        self.logger.info(f"调整渲染管线参数: 最大绘制调用={self.renderer.max_draw_calls}, "
                       ...
                       f"阴影贴图分辨率={self.renderer.shadow_map_res}, "
                       f"LOD偏差={self.renderer.lod_bias}")
        
        if hasattr(self.renderer, "_apply_pipeline_optimizations"):
            self.renderer._apply_pipeline_optimizations(getattr(self.renderer, "curr_pipeline", None))
    ...
    def _adjust_other_features(self):
        ...
        # 可变速率着色支持
        self.renderer.vrs_enabled = render_features.get("variable_rate_shading", False)
    ...
    def get_optimization_report(self):
        ...
        return {
            ...
            "shadow_map_resolution": self.renderer.shadow_map_res,
