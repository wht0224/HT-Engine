...
class GPUCapabilityEvaluator:
    ...
    def _calculate_performance_score(self):
        ...
        vram = int(self.hardware_info.get('vram_mb') or 0)
        architecture = self.hardware_info['gpu_architecture']
        vendor = self.hardware_info['gpu_vendor']
        has_discrete_gpu = bool(self.hardware_info.get("has_discrete_gpu", False))
        ...
        # 2. VRAM大小调整
        is_integrated = (vendor == "Intel") or str(architecture or "").startswith("intel_")
        if is_integrated and not has_discrete_gpu:
            vram_factor = min(max(vram / 1024, 0.25), 1.0)
        else:
            vram_factor = min(max(vram / 2048, 0.25), 3.0)
        ...
        # 5. 最终评分计算
        integrated_penalty = 0.85 if is_integrated and not has_discrete_gpu else 1.0
        self.performance_score = base_score * vram_factor * arch_factor * feature_factor * integrated_penalty
