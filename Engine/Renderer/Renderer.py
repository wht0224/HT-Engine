...
class Renderer:
    ...
    def render(self, scene):
        ...
        try:
            ...
            # 强制调用特效管理器，不管结果是不是 None
            # 这样即使 ForwardRenderer 返回 None，特效也能生效
            color_buffer = None
            depth_buffer = None
            ...
            # 应用特效（只传2个参数）
            final_buffer = self.effect_mgr.render(
                color_buffer, 
                depth_buffer
            )
            ...
            if final_buffer is not None:
                ...
                # 如果没有最终缓冲区，说明已经直接渲染到屏幕了（如ForwardRenderer）
                # 不需要额外操作
                pass
    ...
    def _render_to_screen(self, texture):
        """
        将纹理渲染到屏幕
        
        Args:
            texture: 要渲染的纹理（numpy数组或纹理对象）
        """
        # 如果是numpy数组，需要创建纹理并渲染
        # 这里简化处理，暂时不做任何操作
        # 因为ForwardRenderer已经直接渲染到屏幕了
        pass
    ...
    def render_sky(self, camera):
        ...
        try:
            self.curr_pipeline.render_sky(camera)
        except Exception as e:
            from Engine.Logger import get_logger
            logger = get_logger("Renderer")
            logger.error(f"渲染天空失败: {e}")
    ...
    def _apply_pipeline_optimizations(self, pipeline):
        ...
        if self.quality == RenderQuality.ULTRA_LOW:
            ...
            if hasattr(pipeline, 'set_ssr_quality'):
            ...
            if hasattr(pipeline, 'set_volumetric_quality'):
    ...
    def _prepare_batch_rendering(self, scene):
        ...
        Args:
            scene: 场景对象
        """
        # 暂时禁用批处理系统，以避免创建过多实例批次
        # 这是为了解决每帧重复创建批次导致的性能问题
        pass

    def _update_rendering_features(self):
        """
        ...
        # 针对低端GPU进行优化：大幅减少昂贵的渲染特效
        # 根据VRAM容量决定特效启用策略
        vram_mb = self.hw_info.get('vram_mb', 4096)

        # 对于VRAM较少的GPU，禁用昂贵的特效
        if vram_mb <= 4096:  # 如GTX 1650 (4GB) 或更小
            # 禁用最昂贵的特效
            self.curr_pipeline.enable_feature("ssr", False)  # 屏幕空间反射
            self.curr_pipeline.enable_feature("volumetric_lighting", False)  # 体积光照
            self.curr_pipeline.enable_feature("volumetric_fog", False)  # 体积雾

            # 降低其他特效的质量
            self.curr_pipeline.enable_feature("ao", True)  # 保持环境光遮蔽启用但降低质量
            self.curr_pipeline.enable_feature("bloom", True)  # 保持bloom启用
            self.curr_pipeline.enable_feature("motion_blur", False)  # 禁用运动模糊
            self.curr_pipeline.enable_feature("dof", False)  # 禁用景深

            # 降低阴影质量
            if hasattr(self.curr_pipeline, 'set_shadow_quality'):
                self.curr_pipeline.set_shadow_quality("low")
        else:
            # VRAM充足的GPU可以启用更多特效
            enable_ssr = self.quality in [RenderQuality.MEDIUM, RenderQuality.HIGH] and self.ssr_enabled
            self.curr_pipeline.enable_feature("ssr", enable_ssr)
            if enable_ssr:
                # 根据渲染质量设置SSR质量
                ssr_quality = "medium" if self.quality == RenderQuality.MEDIUM else "high"
                self.curr_pipeline.set_ssr_quality(ssr_quality)

            # 仅在渲染质量较高且GPU支持时启用体积光照
            enable_volumetric = self.quality == RenderQuality.HIGH and self.volumetric_lighting_enabled
            self.curr_pipeline.enable_feature("volumetric_lighting", enable_volumetric)
            if enable_volumetric:
                self.curr_pipeline.set_volumetric_quality("medium")

            # 仅在渲染质量较高且GPU支持时启用环境光遮蔽
            enable_ao = self.quality in [RenderQuality.MEDIUM, RenderQuality.HIGH] and self.ao_enabled
            self.curr_pipeline.enable_feature("ao", enable_ao)
            if enable_ao:
                ao_quality = "medium" if self.quality == RenderQuality.MEDIUM else "high"
                self.curr_pipeline.set_ao_quality(ao_quality)