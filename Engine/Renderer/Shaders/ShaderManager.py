...
struct PBRMaterial {
    ...
    highp float ao;
    sampler2D u_baseColorTexture;     // 基础颜色纹理
    sampler2D u_roughnessTexture;     // 粗糙度纹理
    sampler2D u_metallicTexture;      // 金属度纹理
    sampler2D u_normalTexture;        // 法线纹理
    sampler2D u_aoTexture;            // 环境光遮蔽纹理
    bool useBaseColorTexture;       // 是否使用基础颜色纹理
    bool useRoughnessTexture;       // 是否使用粗糙度纹理
    bool useMetallicTexture;        // 是否使用金属度纹理
    bool useNormalTexture;          // 是否使用法线纹理
    bool useAoTexture;              // 是否使用环境光遮蔽纹理
};
...
highp vec3 calculate_pbr_lighting(
    ...
    // 从纹理中获取材质参数（如果启用了纹理）
    highp vec3 baseColor = material.baseColor;
    highp float roughness = material.roughness;
    highp float metallic = material.metallic;
    highp float ao = material.ao;

    if (material.useBaseColorTexture) {
        baseColor = texture2D(material.u_baseColorTexture, uv).rgb;
    }

    if (material.useRoughnessTexture) {
        roughness = texture2D(material.u_roughnessTexture, uv).r;
    }

    if (material.useMetallicTexture) {
        metallic = texture2D(material.u_metallicTexture, uv).r;
    }

    if (material.useAoTexture) {
        ao = texture2D(material.u_aoTexture, uv).r;
    }
    ...
    // 计算法线分布项和几何遮挡项
    highp float NDF = distribution_ggx(N, H, roughness);
    ...
    // 计算光照贡献
    highp float NdotL = max(dot(N, L), 0.0);
    highp vec3 Lo = (kD * baseColor / 3.1415926535 + specular) * radiance * NdotL;
...
highp vec3 calculate_ambient_lighting(
    ...
    highp vec3 prefilteredColor = textureLod(prefilterMap, R, roughness * MAX_REFLECTION_LOD).rgb;

    // 采样BRDF查找表
    highp vec2 brdf = texture2D(brdfLUT, vec2(max(dot(N, V), 0.0), roughness)).rg;
    highp vec3 specular = prefilteredColor * (kS * brdf.x + brdf.y);

    // 组合环境光照
    highp vec3 ambient = (kD * diffuse + specular) * ao;
